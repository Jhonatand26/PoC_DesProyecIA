...
def build_prompt(class_name: str, confidence: float) -> str:
    ...
    Args:
        ...
        confidence (float): Confianza de la clasificacion (0.0 a 1.0).
    ...
    prompt = (
        f"Eres un agronomo experto en cultivos del Valle del Cauca, Colombia.\n"
        f"Un agricultor consulta sobre una enfermedad detectada en su cultivo.\n\n"
        f"DIAGNOSTICO DETECTADO:\n"
        f"- Enfermedad/Condicion: {clase_legible}\n"
        f"- Confianza del diagnostico: {confianza_porcentaje}%\n\n"
        f"Proporciona una respuesta estructurada con estas secciones:\n\n"
        f"1. DIAGNOSTICO: Que es esta enfermedad y como afecta al cultivo.\n"
        f"2. TRATAMIENTO: Pasos concretos incluyendo productos en Colombia.\n"
        f"3. PREVENCION: Medidas adaptadas al clima del Valle del Cauca.\n\n"
        f"Responde en espanol, lenguaje claro para agricultores. Maximo 300 palabras."
    )
